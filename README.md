...
- **外观与状态调整**：
  ...
  - **宽高自由调节**：按住 `Ctrl + Alt` 并在任意窗口上**滚动鼠标滚轮**可调节窗口高度；按住 `Shift + Alt` **滚动鼠标滚轮**可调节窗口宽度。
...
首次开启自启动时需要确认一次 UAC。程序会为当前用户创建登录任务，并直接记录当前正在运行的
`SmartWindowTool.exe` 完整路径；任务权限会跟随“管理员权限”开关，后续登录不会重复弹出 UAC。
启用自启动后请不要移动或删除该 EXE，否则登录任务将无法启动程序。
